...
class Ctx_forcejs(Ctx_base):
    ...
    def response(self, flow):
        ...
        if re.search(r"(app|main|index)\.[a-zA-Z0-9]{1,}\.js",flow.request.url):
            # 随便写写
            rss=re.findall("[\"'](chunk-[a-zA-Z0-9]{1,})[\"']\:\s?[\"']([a-zA-Z0-9]{1,})[\"']",Ctx_base.autocode(flow.response,flow.response.raw_content))
...
class Ctx_packeaziler(Ctx_base):
    ...
    class __ast(AST):
        ...
        def visit_FunctionExpression(self, node): # 遍历方式充满了一种幽默感，评价是不如traverse
            if((node.id is None) and (len(node.params) == 3)):
                for i in range(len(node.body.body)):
                    if(isinstance(node.body.body[i].expression,CallExpression) and isinstance(node.body.body[i].expression.callee, Identifier) and node.body.body[i].expression.callee.name == "eval"):
                        node.body.body[i]=esprima.parseScript(node.body.body[i].expression.arguments[0].value)
    ...
    def response(self,flow):
        ...
        if re.search(r"(app|main|index).*\.js",flow.request.url): #这种特征其实还不是很明晰 先就这么写 有问题直接私聊我
            if len(re.findall(r"[\/][\*]{2,}[\/]",content))>5:
                flow.response.set_content(Ctx_packeaziler.__ast(content).jsafter.encode("utf8"))